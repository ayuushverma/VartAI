    ...
    <main className="v-page text-white">
      <section className="v-shell mx-auto min-h-[calc(100vh-3rem)] max-w-4xl">
        ...
        <div className="space-y-4 p-5 sm:p-8">
          ...
          <form action={logoutAction}><button className="v-button-secondary w-full px-5 py-3 text-sm font-semibold text-slate-200 sm:w-auto" type="submit">Log out</button></form>
        </div>
      </section>
    </main>
  );
...
async function getProfile() { try { const userId = await getCurrentUserId(); if (!userId) return null; const conversationCount = await db.conversation.count({ where: { userId } }); return { id: userId, conversationCount }; } catch (error) { console.error("Settings data retrieval failed:", error); return null; } }